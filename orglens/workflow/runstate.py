...
def workflow_version(path: Path) -> str:
    ...
    `orchestrator.record` stamps this value onto the completion and gate
    facts it writes — a note of which definition was in force when the fact
    was written, nothing more. Nothing in this engine reads the stamp back:
    no comparison against the current definition, no check, no invalidation
    of a cursor written under an earlier one. It is bookkeeping only, until
    something is built to consult it.
    """
